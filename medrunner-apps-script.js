...
function submitEquipmentRequest(e) {
  try {
    ...
    if (!requestData.discordUsername || !requestData.items.length) {
...
function initializeOrderTracking() {
  ...
  if (!trackingSheet) {
    ...
    const headers = [
      ...
      'Completed by',       // B - Column J on Orders tab
    ...
    trackingSheet.setColumnWidth(2, 150); // Completed by
...
function updateOrderTracking(orderId, completedBy = '', status = '', isNewOrder = false) {
  const ss = SpreadsheetApp.openById(CONFIG.SHEET_ID);
  let trackingSheet = ss.getSheetByName('Order Tracking');
  ...
  if (rowIndex === -1) {
    ...
    if (isNewOrder) {
      trackingSheet.getRange(rowIndex, 4).setValue(new Date());
    }
  }
  ...
  if (completedBy) {
    const currentCompletedBy = trackingSheet.getRange(rowIndex, 2).getValue();
    trackingSheet.getRange(rowIndex, 2).setValue(completedBy);

    if (completedBy && completedBy.trim() !== '' && !currentCompletedBy) {
      trackingSheet.getRange(rowIndex, 5).setValue(now);
    }
  }
    ...
    switch (status) {
      ...
      case 'Pending Collection':
        if (!trackingSheet.getRange(rowIndex, 6).getValue()) {
          trackingSheet.getRange(rowIndex, 6).setValue(now);
        }
        break;
      ...
      case 'Cancelled':
        if (!trackingSheet.getRange(rowIndex, 7).getValue()) {
          trackingSheet.getRange(rowIndex, 7).setValue(now);
        }
        break;
    }
  }

  console.log(`Order tracking updated: ${orderId} - ${completedBy} - ${status}`);
}
...
function populateExistingOrderTracking() {
  ...
  for (let i = 1; i < orders.length; i++) {
    ...
    const completedBy = orders[i][9];
    const status = orders[i][10];

    if (orderId) {
      updateOrderTracking(orderId, completedBy, status, true);
